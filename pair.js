...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/DeN6BbafcWwCCXVKaiSx0s?mode=gi_t',
  RCD_IMAGE_PATH: 'https://files.catbox.moe/6ypccv.jpg',
  NEWSLETTER_JID: '120363424624161926@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94726262058',
  ...
  BOT_FOOTER: 'ɢᴘᴛ ᴍɪɴɪ',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/6ypccv.jpg' }
};
...
// ========== END WORK TYPE RESTRICTIONS ==========
    ...
    let botName = cfg.botName || '🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥';
...
case 'translate': {
    ...
    const botName = cfg.botName || '🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥';
...
case 'alive': {
  try {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'ɢᴘᴛ ᴍɪɴɪ';
...
case 'bots': {
  try {
    const sanitized = (sender || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'ɢᴘᴛ ᴍɪɴɪ';
    const prefix = config.PREFIX;
        ...
        if (thumb) {
            await socket.sendMessage(
                sender,
                {
                    image: { url: thumb },
                    caption:
                        `*🎥 🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥 Video Downloader 💗*\n\n` +
                        `*📍 Title :* _${videoInfo.title || query}_\n\n` +
                        `> Powered by 🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥`,
                },
                { quoted: msg }
            );
        }
...
case 'download': {
  ...
  try {
    ...
    const botName = userCfg.botName || 'ɢᴘᴛ ᴍɪɴɪ';
    const prefix = config.PREFIX;
...
case 'owner': {
    try {
        ...
        let vcard = 
            ...
            'TITLE:ꜰᴏᴜɴᴅᴇʀ & ᴄᴇᴏ ᴏꜰ ɢᴘᴛ ᴍɪɴɪ ʙᴏᴛ;\n' + 
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> 🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥',
            buttons: buttons,
             headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
END:VCARD` } }
  ...
  if (!jid) {
    ...
    const title = userCfg.botName || '🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥';
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname 🐦‍🔥 ɢᴘᴛ ᴍɪɴɪ ᴠ1 🐦‍🔥 - 01`' }, { quoted: shonux });
  }
